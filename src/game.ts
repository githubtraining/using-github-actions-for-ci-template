export default function Game (p1: string, p2: string) {
  this.p1 = p1
  this.p2 = p2
  ...
  this.turn = (row: number, col: number): void => {
    col = col || row
    this.board[row][col] = this.sym
  }

  this.nextPlayer = (): void => {
    this.player = this.player === p1 ? p2 : p1
    this.sym = this.sym === 'X' ? 'O' : 'X'
  }

  this.hasWinner = function (): boolean {
    return rowWin() || colWin() || diagWin()
  }

  const rowWin = (): boolean => {
    let win = false
    for (let r = 0; r < 3; r++) {
